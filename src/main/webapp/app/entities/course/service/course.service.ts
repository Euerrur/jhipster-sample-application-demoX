...
export class CourseService {
  ...
  protected convertDateFromClient(course: ICourse): ICourse {
    return Object.assign({}, course, {
      classTime: course.classTime?.isValid() ? course.classTime.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.classTime = res.body.classTime ? dayjs(res.body.classTime) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((course: ICourse) => {
        course.classTime = course.classTime ? dayjs(course.classTime) : undefined;
      });
    }
    return res;
  }
